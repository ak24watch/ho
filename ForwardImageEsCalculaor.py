...
# Plot x (the circular object) in 128x128 resolution
plt.figure(figsize=(6, 6))
x_plot = np.reshape(x, (L_circ, L_circ))
x_plot_128 = np.zeros((128, 128), dtype=np.complex128)
start = (128 - L_circ) // 2
x_plot_128[start:start+L_circ, start:start+L_circ] = x_plot

plt.imshow(np.real(x_plot_128), cmap='viridis', extent=[0, 128, 0, 128])
plt.title('Real part of x (128x128)')
plt.xlabel('x')
plt.ylabel('y')
plt.colorbar(label='Real(x)')
plt.show()
...
y, _ = util_cgfft.cg_fft_forward_problem(x, G_S_forward, g_D_fft_forward, e_forward, 1e-7, e_forward, 1000)
...
# Plot real part of y at receiver positions
plt.figure(figsize=(8, 4))
plt.subplot(1, 2, 1)
plt.scatter(pos_S[:, 0], pos_S[:, 1], c=np.real(y.flatten()), cmap='viridis')
plt.colorbar(label='Real(y)')
plt.title('Real part of y at Receiver Positions')
plt.xlabel('x (m)')
plt.ylabel('y (m)')

# Plot imaginary part of y at receiver positions
plt.subplot(1, 2, 2)
plt.scatter(pos_S[:, 0], pos_S[:, 1], c=np.imag(y.flatten()), cmap='RdBu')
plt.colorbar(label='Imag(y)')
plt.title('Imaginary part of y at Receiver Positions')
plt.xlabel('x (m)')
plt.ylabel('y (m)')
